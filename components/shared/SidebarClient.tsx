    ...
    <>
      ...
      `}>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
          {NAV_ITEMS.map((item) => {
            if (!user && item.href === "/favorites") return null;
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-3 px-4 py-3 transition-all ${
                  isActive 
                    ? "sidebar-item-active text-primary rounded-lg" 
          ...
          {user && profile?.show_h_content && (
            <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-800">
              <Link
                ...
                onClick={() => setIsOpen(false)}
              >
                <span className="material-symbols-outlined" style={pathname === "/hentai" ? { fontVariationSettings: "'FILL' 1" } : undefined}>
                  lock
                </span>
                <span className="font-medium text-sm">Hentai</span>
              </Link>
            </div>
          )}
        </nav>
