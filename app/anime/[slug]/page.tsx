...
export default async function AnimeDetailPage({ params }: Props) {
  ...
  // Extract ID from slug (e.g., "123-title" -> 123)
  ...
  return (
    <div className="flex-1 min-h-screen relative px-4 sm:px-8 lg:px-12 pb-20">
      {/* Hero Background (Blurred) - Contained */}
      ...
      <div className="relative z-10 p-6 lg:p-12 max-w-7xl mx-auto glass-card rounded-3xl mt-24 lg:mt-32">
        {/* Breadcrumbs */}
        <div className="flex items-center gap-2 text-xs font-medium text-text-muted mb-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="w-full lg:w-[320px] shrink-0">
            <div className="rounded-2xl overflow-hidden shadow-2xl shadow-black/50 border border-white/10 group">
              <img 
          ...
          <div className="flex-1 py-2">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              {anime.rank && (
                <span className="px-2 py-1 rounded bg-primary/20 text-primary text-[10px] font-bold tracking-wider uppercase">
                  Rank #{anime.rank}
                </span>
              )}
              <span className="px-2 py-1 rounded bg-white/5 text-text-muted text-[10px] font-bold tracking-wider uppercase">
                {anime.type}
              </span>
              <span className="px-2 py-1 rounded bg-white/5 text-text-muted text-[10px] font-bold tracking-wider uppercase">
                 {anime.status}
            ...
            <div className="flex flex-wrap gap-6 mb-8">
              ...
              <div className="flex flex-col">
                <span className="text-lg font-bold text-text-primary">{anime.season || 'N/A'} {anime.year}</span>
                <span className="text-xs text-text-muted">Premiere</span>
              </div>
              <div className="flex flex-col">
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {anime.genres.map(genre => (
                <span key={genre.mal_id} className="px-4 py-1.5 rounded-full bg-white/5 text-sm font-medium text-text-primary border border-white/5 hover:border-primary/30 transition-colors">
                  {genre.name}
                </span>
              ))}
            </div>
            ...
            <div className="relative group">
              <p className="text-text-muted text-lg leading-relaxed mb-10 max-w-3xl line-clamp-6 group-hover:line-clamp-none transition-all duration-300">
            ...
            <div className="flex flex-wrap gap-4">
              <button className="px-8 py-4 bg-primary text-white font-bold rounded-xl flex items-center gap-2 shadow-xl shadow-primary/30 hover:bg-primary/90 transition-all cursor-pointer">
                <span className="material-symbols-outlined">add</span>
                Add to List
              </button>
              <button className="p-4 rounded-xl text-text-primary hover:bg-white/10 transition-all glass-card cursor-pointer">
          ...
          <section className="mt-20">
            ...
            <div className="flex flex-col gap-3">
              {recentEpisodes.map(ep => (
                <div key={ep.mal_id} className="p-4 rounded-2xl flex items-center gap-4 hover:bg-white/5 transition-all group cursor-pointer border border-transparent hover:border-white/5">
                  <div className="w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                    <span className="text-primary font-bold">{ep.mal_id}</span>
                  </div>
